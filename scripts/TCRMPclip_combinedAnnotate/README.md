...
The review topbar's **Import Images** button brings ANY operator-supplied
image (jpg or png) into the active session, alongside the pipeline-routed
frames. Server side this is `POST /api/import_images` (multipart field
`images`, or JSON `{"paths": [...]}` for files already on this machine),
implemented in `src/custom_imports.py`:

- The original file is copied verbatim into
  `<export_dir>/custom_imports/originals/`.
- The delivered annotation image lands in `custom_imports/raw/` with a
  `custom_` filename prefix. Any import whose long edge exceeds 1920px is
  downscaled to the lores standard (same rule as pipeline frames, because SAM3
  resizes to 1008px and full 4K input produces junk masks); the delivered
  file gains a `_lores` suffix and the manifest records the scale, so a
  coordinate on the delivered image maps back to the original as
  `(x / scale, y / scale)`. EXIF orientation is baked into the delivered
  pixels so the browser canvas and cv2 agree.
- `custom_imports/imports_manifest.json` records every import (dims, scale,
  AST timestamp, reviewer); on a later configure/resume the manifest backfills
  any import missing from the segmentations store.
- The run's `project.json` gets an `access_log` entry (`action:
  import_images`) plus a cumulative `custom_imports` list.
- Imported frames join the review queue immediately, carry a magenta
  **Custom** badge in the topbar, and flow through the normal batch export
  (YOLO bundle, manifest, mask registry) exactly like routed frames. Nothing
  is ever written into `routed_input/`, so routing invalidation cannot drop
  an import.

Tests: `tests/test_custom_imports.py` (registered in
