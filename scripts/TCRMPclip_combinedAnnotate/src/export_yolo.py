...
def export_batch(segmentations, export_dir, class_map, symlink=True):
    ...
    for filename, seg in segmentations.items():
        ...
        # content-stable uid. Runs before any skip guard below (scrapped,
        # image-not-found, review-only, not-accepted) so a frame whose masks
        # are all rejected/review/scrapped still lands in the registry.
        # Never fatal to the export.
        _upsert_registry_for_frame(filename, seg, export_dir)

        # Belt-and-suspenders: a scrapped frame must never reach the training
        # export, even if it somehow ends up in to_export. do_export already
        # records it as 'scrapped' in the manifest and excludes it from disk.
        if seg.get('scrapped'):
            continue
        ...
        all_masks = seg.get('masks', [])
        accepted = [m for m in all_masks
        ...
        # label). The server-side accept guard should prevent this, but a
        # mask could still reach here via another path (e.g. a direct
        # commit). Hard-skip it here and count it so nothing vanishes
        # unreported.
        ...
        if not accepted:
            # Review-only frame: coral is present but flagged for expert review,
            # so it must NOT enter training as a (false) negative. It ships to
            # the expert repo via _export_review_bundle. Skip the training set.
            has_review = any(m.get('review') or m.get('species') == 'REVIEW'
                             for m in all_masks)
            if has_review:
                review_only_skipped += 1
                continue
            # Genuine negative: nothing found here. Export image + empty label.
        ...
        if accepted:
            ...
            exported_empty += 1
            exported_images += 1  # negatives count as exported frames
    ...
    with open(log_path, 'a') as f:
        f.write(f'{datetime.now().isoformat()} — exported {exported_images} images, '
                f'{exported_masks} masks, {exported_empty} negatives, '
                f'{blocked_unlabeled} blocked-unlabeled, '
                f'{new_classes} new classes\n')

    stats = {
        'exported_images': exported_images,
        'exported_masks': exported_masks,
        'exported_empty': exported_empty,
        'review_only_skipped': review_only_skipped,
