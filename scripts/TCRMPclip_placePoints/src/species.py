"""
Species lookup - joins OCR-detected point labels with species codes from all_points.csv.

Copied from TCRMPclip_cpcID/src/species.py with OCR-specific helpers added.

Handles fuzzy date matching for cases where the filename date
...
def ocr_date_to_lookup_date(date_str):
    """Convert YYYYMMDD to YYYY-MM-DD for species lookup.
...
def parse_transect_frame(t_id):
    """Decompose transect ID string into (transect, frame).

    The t_id from filenames like T101 (after stripping the T) encodes:
    - First digit(s): transect number
    - Last two digits: frame number

    Args:
        t_id: "101" -> transect 1, frame 1
              "315" -> transect 3, frame 15
              "622" -> transect 6, frame 22
    Returns:
        (transect: int, frame: int)
    """
    if len(t_id) >= 3:
        frame = int(t_id[-2:])
        transect = int(t_id[:-2])
