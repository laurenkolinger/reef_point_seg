...
def render_segmentation_overlays(seg_dict, filename, export_dir, year, stage="auto"):
    """Render overlay for a single image and save to the appropriate directory.

    Args:
        seg_dict: segmentation dict for one image
        filename: image filename (e.g., "TCRMP20241022_clip_FLC_T110.jpeg")
        export_dir: root export directory
        year: year int
        stage: 'auto' (after SAM3 processing) or 'reviewed' (after export)

    Saves to: export_dir/segmentations/{year}/overlays_{stage}/{stem}_seg.jpg
    """
    ...
    overlay_dir = os.path.join(export_dir, 'segmentations', str(year), f'overlays_{stage}')
